# MOSAIC Source Directory

This directory contains all operational files for the MOSAIC platform. The `src/` directory is designed to be deployed as the complete application to a web server.

## Directory Structure

```text
src/
├── setup.php                    # Database initialization script
├── config/
│   └── config.yaml              # Application configuration (generated by setup)
├── database/
│   └── schema.sql               # Complete MySQL schema
├── scripts/
│   ├── create_admin_user.php    # Admin user creation script
│   └── cleanup_logs.php         # Log cleanup maintenance script
├── Core/
│   ├── Config.php               # Configuration loader
│   ├── Logger.php               # Logging system
│   └── Model.php                # Base model class (planned)
├── Models/                      # Data models (planned)
├── controllers/                 # Request handlers (planned)
├── views/                       # Templates (planned)
└── Exceptions/                  # Custom exceptions (planned)
```

## Deployment

The entire `src/` directory can be deployed to a web server. This is the **only directory** that should be deployed to production.

### Recommended Server Configuration

#### Apache (.htaccess in src/)

```apache
# Deny access to sensitive files
<FilesMatch "(config\.yaml|setup\.php|\.sql)$">
    Require all denied
</FilesMatch>

# PHP settings
php_value upload_max_filesize 10M
php_value post_max_size 10M
php_value memory_limit 128M
```

#### Nginx

```nginx
location ~ (config\.yaml|setup\.php|\.sql)$ {
    deny all;
}

location ~ \.php$ {
    fastcgi_pass unix:/var/run/php/php8.1-fpm.sock;
    fastcgi_index index.php;
    include fastcgi_params;
    fastcgi_param SCRIPT_FILENAME $document_root$fastcgi_script_name;
}
```

## Initial Setup

### 1. Database Initialization

```powershell
# From project root
php src/setup.php
```

This creates:

- Database with UTF8MB4 charset
- All tables (24 tables including logging)
- Standard roles
- `src/config/config.yaml` with database credentials

### 2. Create Admin User

```powershell
# From project root  
php src/scripts/create_admin_user.php
```

### 3. Start Development Server

```powershell
# From project root
php -S localhost:8000 -t src/
```

Access at: `http://localhost:8000/`

## Security Considerations

### File Permissions (Production)

```bash
# Restrict config file
chmod 600 src/config/config.yaml

# Scripts should not be web-accessible
chmod 700 src/scripts/
chmod 700 src/setup.php

# Web-accessible directories
chmod 755 src/
chmod 644 src/index.php  # When created
```

### Environment-Specific Configuration

Edit `src/config/config.yaml`:

#### Development

```yaml
app:
  debug: true
  
session:
  cookie_secure: false  # Non-HTTPS local development
```

#### Production

```yaml
app:
  debug: false
  
session:
  cookie_secure: true   # HTTPS only
```

### Disable Direct Script Access (Production)

After initial setup, you should:

1. **Move setup.php outside webroot** or delete it
2. **Move scripts/ outside webroot** - run via CLI/cron only
3. **Restrict database/ access** via web server configuration
4. **Secure config directory** - no web access

Example production structure:

```text
/opt/mosaic/
  └── src/           # Webroot points here
      ├── index.php  # Public entry point
      ├── config/    # Protected by .htaccess
      └── ...
```

## Maintenance Scripts

### Log Cleanup

Run periodically (weekly recommended):

```powershell
php src/scripts/cleanup_logs.php 90  # Keep 90 days
```

#### Windows Task Scheduler

```powershell
$action = New-ScheduledTaskAction -Execute 'php' -Argument 'C:\path\to\mosaic\src\scripts\cleanup_logs.php 90'
$trigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Sunday -At 2am
Register-ScheduledTask -TaskName "MOSAIC Log Cleanup" -Action $action -Trigger $trigger
```

#### Linux Cron

```bash
0 2 * * 0 /usr/bin/php /var/www/mosaic/src/scripts/cleanup_logs.php 90
```

## Additional Documentation

- **Setup Guide**: `../setup/README.md`
- **Schema Documentation**: `../docs/concepts/SCHEMA.md`
- **Security Guide**: `../docs/concepts/SECURITY.md`
- **Logging Guide**: `../docs/implementation/LOGGING.md`
- **Architecture**: `../docs/concepts/ARCHITECTURE.md`
