...
$logFile = $rootDir . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'setup_' . date('Y-m-d_His') . '.log';
...
// Get file paths (all operational files in src/)
$schemaFile = __DIR__ . DIRECTORY_SEPARATOR . 'database' . DIRECTORY_SEPARATOR . 'schema.sql';
$configFile = __DIR__ . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'config.yaml';
...
if (!test_connection($host, $user, $pass)) {
    color_output("[X] Connection failed. Please check your credentials.", 'red');
    exit(1);
}

color_output("[OK] Connection successful", 'green');
echo PHP_EOL;
...
if (!create_database($host, $user, $pass, $dbname)) {
    color_output("[X] Failed to create database", 'red');
    exit(1);
}

color_output("[OK] Database created (or already exists)", 'green');
echo PHP_EOL;
...
if (!execute_schema($host, $user, $pass, $dbname, $schemaFile)) {
    color_output("[X] Failed to execute schema", 'red');
    exit(1);
}

color_output("[OK] Schema executed successfully", 'green');
echo PHP_EOL;
...
if (!save_config($host, $user, $pass, $dbname, $port, $configFile)) {
    color_output("[X] Failed to save configuration", 'red');
    exit(1);
}

color_output("[OK] Configuration saved to: $configFile", 'green');
echo PHP_EOL;
...
echo "  1. Create an admin user (see src/scripts/create_admin_user.php)" . PHP_EOL;
